      ...
      <div className="p-10 grid grid-cols-1 lg:grid-cols-2 gap-12">
         ...
         <div className="space-y-6">
            <div className="flex items-center gap-3 border-b border-slate-100 pb-4">
               ...
               <h3 className="text-xs font-black text-horvath-900 uppercase tracking-widest">Critical Priorities</h3>
            </div>
            <div className="space-y-4">
               {model.priorities.map((p, idx) => (
                  <div key={idx} className="flex gap-4 p-4 rounded-xl bg-slate-50/50 border border-slate-100">
                     <span className="text-sm font-black text-horvath-700 font-mono">{idx + 1}.</span>
                     <div>
                        <h4 className="text-sm font-black text-horvath-900 uppercase mb-1">{p.title}</h4>
         ...
         <div className="space-y-6">
            <div className="flex items-center gap-3 border-b border-slate-100 pb-4">
               ...
               <h3 className="text-xs font-black text-horvath-900 uppercase tracking-widest">Intervention Matrix</h3>
            </div>
            <div className="grid grid-cols-1 gap-3">
               {model.quickWins.map((win, idx) => (
                  <div key={idx} className="flex items-center gap-3 px-4 py-3 rounded-lg bg-white border border-slate-100 text-xs font-bold text-slate-500 uppercase tracking-tight">
                     <div className="w-1.5 h-1.5 bg-horvath-700 rounded-full" />
                     {win}
                  </div>
               ))}
            </div>
            ...
            {model.notes.length > 0 && (
               <div className="mt-8 p-6 bg-horvath-900 text-white rounded-xl shadow-md flex gap-4 items-start">
                  <ShieldAlert className="w-5 h-5 text-horvath-700 shrink-0" />
